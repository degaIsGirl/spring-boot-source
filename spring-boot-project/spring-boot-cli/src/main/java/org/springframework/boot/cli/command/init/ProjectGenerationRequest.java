...
class ProjectGenerationRequest {
	...
	/**
	 * The URL of the services to use.
	 ...
	 * services supports. If not set, the default is retrieved from the services metadata.
	 ...
	 * @param metadata the metadata that describes the services
	...
	URI generateUrl(InitializrServiceMetadata metadata) {
		try {
			...
			throw new ReportableException("Invalid services URL (" + ex.getMessage() + ")");
		}
	}
		...
		if (this.type != null) {
			...
			if (result == null) {
				throw new ReportableException(
						("No project type with id '" + this.type + "' - check the services capabilities (--list)"));
			}
			return result;
		}
			...
			if (types.size() == 1) {
				...
				throw new ReportableException("No type found with build '" + this.build + "' and format '" + this.format
						+ "' check the services capabilities (--list)");
			}
			else {
