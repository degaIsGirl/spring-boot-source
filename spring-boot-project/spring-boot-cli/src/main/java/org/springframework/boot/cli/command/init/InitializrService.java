...
/**
 * Invokes the initializr services over HTTP.
 *
...
class InitializrService {
	...
	/**
	 * Accept header to use to retrieve the services capabilities of the services. If the
	 * services does not offer such feature, the json meta-data are retrieved instead.
	 */
	...
	ProjectGenerationResponse generate(ProjectGenerationRequest request) throws IOException {
		Log.info("Using services at " + request.getServiceUrl());
	 ...
	 * @param serviceUrl to url of the initializer services
	 * @return the metadata describing the services
	 * @throws IOException if the services's metadata cannot be loaded
	 */
	...
	/**
	 * Loads the services capabilities of the services at the specified URL. If the services
	 ...
	 * @param serviceUrl to url of the initializer services
	 * @return the services capabilities (as a String) or the
	 * {@link InitializrServiceMetadata} describing the services
	 * @throws IOException if the services capabilities cannot be loaded
	 */
	...
	/**
	 * Retrieves the meta-data of the services at the specified URL.
	 * @param url the URL
	 * @return the response
	 */
	...
	private CloseableHttpResponse execute(HttpUriRequest request, Object url, String description) {
		try {
			...
			throw new ReportableException(
					"Failed to " + description + " from services at '" + url + "' (" + ex.getMessage() + ")");
		}
	}

	private ReportableException createException(String url, CloseableHttpResponse httpResponse) {
		String message = "Initializr services call failed using '" + url + "' - services returned "
